...
- 命名方案保存在本机，可快速载入、覆盖和删除。
- 启动时自动检查 GitHub 最新版本；发现更新后可在软件内下载、校验并启动安装。
- 设置页支持关闭启动检查或立即手动检查；关于页可直达 GitHub 仓库和版本发布页面。
- 支持按名称、修改日期、文件大小、文件类型排序，并可倒序。
- 执行前完整预览，可逐项手动修改新名称。
- 打开独立最终预览窗口并再次确认后，才会真正改动磁盘。
...
- 编辑框右键菜单支持撤销、剪切、复制、粘贴、删除和全选；下拉框上的鼠标滚轮不会再误切换选项。
...
2. 下载最新的 `BatchRename-Setup-1.1.2-x64.exe`。
3. 右键安装包并选择“以管理员身份运行”。
