...
try
{
    ...
    if ($OpenFinalPreview)
    {
        Start-Sleep -Milliseconds 400
        $windowElement = [System.Windows.Automation.AutomationElement]::FromHandle($captureProcess.MainWindowHandle)
        $previewCondition = [System.Windows.Automation.PropertyCondition]::new(
            [System.Windows.Automation.AutomationElement]::AutomationIdProperty,
            'FinalPreviewButton')
        $previewButton = $windowElement.FindFirst([System.Windows.Automation.TreeScope]::Subtree, $previewCondition)
        if (-not $previewButton) { throw 'FinalPreviewButton automation element was not found.' }
        $invokePattern = $previewButton.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern)
        $invokePattern.Invoke()
        Start-Sleep -Milliseconds 600
        $processCondition = [System.Windows.Automation.PropertyCondition]::new(
            [System.Windows.Automation.AutomationElement]::ProcessIdProperty,
